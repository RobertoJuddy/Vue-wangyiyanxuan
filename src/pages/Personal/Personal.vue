<template>
    <div class="Personal" v-if="userInfo">
      <div class="PersonalHeader">
        <div class="PersonalHeaderLeft">
          <img src="//yanxuan.nosdn.127.net/8945ae63d940cc42406c3f67019c5cb6.png" />
          <div class="userInfo">
            <span class="phone" v-show="!userInfo.name">{{userInfo.phone}}</span>
            <span class="phone" v-show="!userInfo.phone">{{userInfo.name}}</span>

            <span class="normal">普通会员</span>
          </div>
        </div>
        <div class="PersonalHeaderRight" >
          <i class="iconfont icon-erweima"></i>
          <div class="VIP">
            <i class="iconfont icon-lightningbshandian"></i>
            <div class="SuperVIP">
              <span class="SuperVIPText">超级会员</span>
              <div class="Immediately">
                <span class="ImmediatelyText">立即调用</span>
      ...
      <div class="BScrollPersonal">
        <div class="PersonalContent" v-if="userInfo">
          <ul class="PersonalList">
            <li v-for="(item, index) in info" :key="index">
              <div class="iconfontMain">
                <i class="iconfont" :class=item.icon></i>
              </div>

              <span class="PersonalText">{{item.name}}</span>
            </li>
          </ul>
          <div class="PersonalMainPhone" @click="Logout">
            <span class="PhoneLogin" >登出</span>
          </div>
        </div>
      </div>
...
<script>
  ...
  export default {
   ...
   methods : {
    ...
    // mounted () {
    //   let local =JSON.parse(localStorage.getItem('loginInfo'))
    //
    //   this.$store.dispatch('localStorage' ,local)
    //   if(!local){
    //     this.$router.replace('/login')
    ...
    // },
    computed : {
      ...mapState(['local','userInfo'])
    },
    watch : {
      local () {
        this.$nextTick(()=> {
          new BScroll('.BScrollPersonal', {
            click : true ,
            bounce : true
          })
        })
